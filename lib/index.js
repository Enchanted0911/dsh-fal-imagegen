...
import { FalTaskStore, taskSummary } from './tasks.js'
...
export const FAL_IMAGEGEN_GUIDANCE = [
  '本机已安装 dsh-fal-imagegen 插件（fal.ai 原生生图，宿主半）：Agent 工具 `fal_generate_image`（文生图）、`fal_edit_image`（图生图，参考图传附件引用 source_image 或本机绝对路径 image_path）、`fal_get_image_task`（后台任务查询）、`fal_list_image_models`（列出内置 fal 别名）。',
  `默认模型「gpt-image-2.5-flare」= ${DEFAULT_TEXT_MODEL}（fal 上没有 "flash" 这个名字，flare 就是它的默认快档；图生图为 .../flare/edit）。`,
  'fal_generate_image 默认异步：提交到 fal 队列后立即返回 status/task_id（images 为空），随后用 `fal_get_image_task`（task_id 必填，可传 wait_seconds 最多 120 秒等待出图）查询进度与最终结果；传 wait=true 则改为同步等待返回图片。fal_edit_image 保持同步。',
...
/** Shared output shape: a task snapshot (async submit / query) or a finished batch. */
...
/**
 * Materialize one endpoint response: download/decode every result image,
 * store the batch as attachments, copy the bytes to the local directory.
 *
 * Shared by the synchronous path (fal_edit_image, wait=true) and by the
 * background collectors that finish async tasks.
 */
async function materializeResponse(ctx, config, { slug, mode, source, requestId, response, signal }) {
...
/**
 * Run one endpoint and wait for its full result (the synchronous path): submit,
 * poll the queue ticket, then materialize the response images.
 */
async function runAndCollect(ctx, config, options) {
  ...
  const materialized = await materializeResponse(ctx, config, {
    slug,
    mode,
    source,
    requestId,
...
/** Progress copy for a still-pending task. */
function pendingMessage(record) {
  const position = record.queue_position === undefined ? '' : `（队列第 ${record.queue_position} 位）`
  return `${record.status === 'queued' ? '排队中' : '生成中'}${position}：任务 ${record.request_id} 还在 fal 后台跑，用 fal_get_image_task 继续查。`
}

/**
 * Finish one registered task through its fal ticket: poll until COMPLETED,
 * fetch the result, materialize the images, and write the outcome back into
 * the record. Errors land in the record too (status 'failed') — the record's
 * fal request_id survives a budget timeout, so a later query can retry.
 *
 * Exactly one finish runs per record: the collector started at submit time,
 * or the first query that finds a stranded record. Concurrent callers await
 * the same promise, so materialization happens at most once.
 */
function finishTask(ctx, config, tasks, record, signal) {
  const retryable = record.status === 'failed' && record.error_code === 'fal-timeout'
  if (record.promise !== undefined && !retryable) return record.promise
  // A timed-out record is retried on its next query: replace the settled
  // promise (the status flip below is synchronous, so concurrent callers
  // reuse the fresh one instead of starting a second finisher).
  record.promise = (async () => {
    tasks.update(record.task_id, {
      status: 'queued',
      message: record.error_code === 'fal-timeout' ? '上次等待超时，正在自动续查 fal 结果…' : pendingMessage(record),
    ...
    try {
      for (;;) {
        if (Date.now() >= deadline) {
          throw new FalError(
            `fal 任务 ${record.request_id} 后台等待超时（${Math.round(config.timeoutMs / 1000)}s，仍在排队或生成）。稍后再次调用 fal_get_image_task 会自动续查。`,
            'fal-timeout',
          )
        }
        const tick = await falTicket({
        ...
        // Comp/deleted never publish the terminal state here: a COMPLETED
        // ticket only exits the loop — the record flips to 'completed'
        // atomically with its images after materialization, so a query can
        // never observe "completed" without images.
        const status = tick.status === 'IN_QUEUE' ? 'queued' : 'running'
        tasks.update(record.task_id, {
          status,
          queue_position: tick.queuePosition ?? record.queue_position,
          message: pendingMessage({ ...record, status, queue_position: tick.queuePosition ?? record.queue_position }),
        })
        if (tick.status !== 'IN_QUEUE' && tick.status !== 'IN_PROGRESS') break
        await falSleep(POLL_INTERVAL_MS, signal)
      }
      const { requestId, response } = await falResult({
        requestId: record.request_id,
        responseUrl: record.response_url,
        apiKey: config.falKey,
        signal,
        deadline,
      })
      ...
      const code = error instanceof FalError ? error.code : undefined
      tasks.update(record.task_id, {
        status: 'failed',
        error_code: code,
...
/** The immediate answer `fal_generate_image` returns after submission. */
function submitPayload(record) {
  return {
    ...
    ...record.queue_position === undefined ? {} : { queue_position: record.queue_position },
    message: `任务已提交到 fal 队列（${record.model}）。后台会自动生成，稍后用 fal_get_image_task 查询结果（task_id=${record.task_id}）。`,
    next_action: `调用 fal_get_image_task，参数 task_id="${record.task_id}"`,
    images: [],
  }
}

/** Snapshot of one task record at query time. */
function taskPayload(record) {
  return {
    ...
    message: record.status === 'completed'
      ? record.message
      : (record.status === 'failed' ? `任务失败：${record.error}` : pendingMessage(record)),
    ...record.error === undefined ? {} : { error: record.error },
    ...record.status !== 'completed'
      ? { next_action: `继续调用 fal_get_image_task（task_id=${record.task_id}）直到 status=completed` }
      : {},
...
function registerTools(ctx, resolve, tasks, background) {
  return [
    ctx.tools.register(defineTool({
      name: 'fal_generate_image',
      description:
        'Generate an image through fal.ai (native fal protocol: queue.fal.run + Authorization: Key). '
        + 'By default this is ASYNC: the tool submits the job to the fal queue and returns immediately with a '
        + 'task_id and status "queued" — no images yet. Use fal_get_image_task with that task_id to poll progress '
        + 'and receive the finished images (pass wait_seconds to block up to 120s). Pass wait=true instead to '
        + 'block here until the images are ready (returns them directly). Only fal endpoints are allowed: pass one '
        + 'of the built-in aliases (gpt-image-2.5-flare is the default and equals the fastest GPT Image 2.5 tier, '
        + 'gpt-image-2.5-sunburst, gpt-image-2, gpt-image-1.5, nano-banana-2, gemini-25-flash-image, flux-2-flash) '
        + 'or a raw fal slug containing "/". Call fal_list_image_models to see the catalog.',
      parameters: {
        ...
        wait: { type: 'boolean', description: 'false (default): submit and return a task_id immediately; true: wait for the finished images in this same call.' },
      },
      output: {
      ...
      async execute(args, exec) {
        ...
        // The documented synchronous escape hatch: block for this one call.
        if (args.wait === true) {
        ...
        // Async path: submit, register the task, and let a detached collector
        // finish it. The call returns as soon as fal accepted the job.
        ...
        void finishTask(ctx, config, tasks, record, controller.signal).finally(() => background.delete(controller))
        return submitPayload(record)
      },
    })),
    ctx.tools.register(defineTool({
      name: 'fal_edit_image',
      description:
        ...
        + 'returned by fal_generate_image / fal_edit_image, passed through unchanged) or as image_path (an absolute local file path, e.g. the '
        + 'path returned by a previous generation). The matching fal /edit endpoint is used automatically for built-in aliases. '
        + 'The result renders beside this tool call and is also written to disk. This tool waits for the finished images.',
      parameters: {
    ...
    ctx.tools.register(defineTool({
      name: 'fal_get_image_task',
      description:
        'Query the background fal image task that fal_generate_image submitted (it returns a task_id immediately; '
        + 'images arrive here). Pass task_id (or fal request_id); omit both to list recent tasks instead. '
        + 'The result has status queued / running / completed / failed: when completed the images are attached '
        + 'beside this call and written to disk, with paths in images[].path. Pass wait_seconds (0–120, default 0) '
        + 'to wait up to that long for the task to finish — the waiting happens in this call, so prefer short values '
        + 'and re-query. A task that timed out earlier is retried automatically on its next query.',
      parameters: {
        task_id: { type: 'string', description: 'The task_id returned by fal_generate_image. Omit (along with request_id) to list recent tasks.' },
        request_id: { type: 'string', description: 'Alternatively, fal\'s own request_id from a previous result.' },
        wait_seconds: { type: 'integer', description: 'Block up to this many seconds for completion (0–120; 0 = one quick poll).' },
      },
      output: {
        schema: taskResultSchema,
        render: (_args, value) => renderTaskResult(value),
      ...
      async execute(args, exec) {
        ...
        const taskId = text(args.task_id)
        ...
        if (taskId === '' && requestId === '') {
          ...
          return {
            ...
            message: recent.length === 0
              ? '目前没有后台任务。先用 fal_generate_image 提交一个。'
              : `共 ${recent.length} 个后台任务（最新在前）：${recent.map(record => `${record.task_id}=${record.status}`).join('，')}`,
            tasks: recent.map(taskSummary),
            images: [],
          }
        }
        ...
        const record = taskId !== ''
          ? tasks.get(taskId)
          : tasks.findByRequestId(requestId)
        if (record === undefined) {
          throw new Error(`找不到任务 ${taskId !== '' ? taskId : requestId}。任务记录在本机进程内保存约 6 小时；进程重启后旧任务会丢失（fal 的 request_id 仍可在 fal.ai 控制台查到）。`)
        }

        // Finished: serve the stored outcome (background collector already
        // materialized the images once).
        if (record.status === 'completed') return taskPayload(record)
        if (record.status === 'failed' && record.error_code !== 'fal-timeout') return taskPayload(record)

        // Still pending — or stranded after a timeout: make sure a finisher is
        // attached (single-flight), then wait up to wait_seconds for it.
        ensureConfigured(config)
        finishTask(ctx, config, tasks, record, exec?.signal)
        const wait = Math.min(120, Math.max(0, Math.round(Number(args.wait_seconds) || 0)))
        if (wait > 0) {
          await Promise.race([
            record.promise,
            falSleep(wait * 1000, exec?.signal),
          ])
        } else {
          // One bounded tick so a fast job resolves on the first query too.
          await Promise.race([
            record.promise,
            falSleep(POLL_INTERVAL_MS, exec?.signal),
          ])
        }
        return taskPayload(tasks.get(record.task_id) ?? record)
      },
    })),
    ctx.tools.register(defineTool({
      ...
      async execute() {
        ...
        return {
          ...
          async_note: 'fal_generate_image 默认异步提交并返回 task_id；用 fal_get_image_task 查询进度与结果（可传 wait_seconds 等待）。',
        }
      },
    })),
  ]
...
export function apply(ctx, config) {
  ...
  /** Background task registry + the collectors currently running. */
  const tasks = new FalTaskStore()
  const background = new Set()

  const disposers = registerTools(ctx, resolve, tasks, background)
